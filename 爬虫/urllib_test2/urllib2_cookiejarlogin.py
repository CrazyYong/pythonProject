...
# 参数就是构建的CookieJar()对象
cookie_handler = url_re.HTTPCookieProcessor(cookie)

# 构建一个自定义的opener
opener = url_re.build_opener(cookie_handler)
...
# 通过urlencode()编码转换
data = url_pa.urlencode(data).encode(encoding='UTF8')

# 第一次是post请求，发送登录需要的参数，获取cookie
request = url_re.Request(url, data = data)
